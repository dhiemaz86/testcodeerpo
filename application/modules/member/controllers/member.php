...
/*	function auth(){
        ...
        if($this->session->userdata('online')==true){
            redirect('member/berhasillogin');
        }else{
            echo 'kok raiso iki';
            //redirect('member/gagallogin');
        }
    }*/

    function auth(){
        $username=strip_tags(str_replace("'", "", $this->input->post('nama')));
        $nomeja=strip_tags(str_replace("'", "", $this->input->post('nomeja')));
        $nama=$username;
        $no_meja=$nomeja;
        $cadmin=$this->m_pelanggan->simpan_pembeli($nama,$no_meja);
